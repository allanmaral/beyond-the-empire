...
const NullcheckIcon: React.FC<{
  ...
  >
    <path
      ...
      fill={fill}
    />
    <path d="M12 8H4" stroke={bg} strokeWidth="1.5" />
  </SVG>
)
const UncheckedIcon: React.FC<{ stroke: string; disabled?: boolean }> = ({
  stroke,
  disabled
}) => (
  <SVG
...
const CheckboxIcon: React.FC<CheckboxIconProps> = ({ disabled, checked }) => {
  ...
  if (checked === true) {
    return <CheckedIcon fill={fill} bg={bg} disabled={disabled} />
  }

  if (checked === false) {
    return <UncheckedIcon stroke={stroke} disabled={disabled} />
  }

  return <NullcheckIcon fill={fill} bg={bg} disabled={disabled} />
}

const MemoCheckboxIcon = memo(CheckboxIcon)
